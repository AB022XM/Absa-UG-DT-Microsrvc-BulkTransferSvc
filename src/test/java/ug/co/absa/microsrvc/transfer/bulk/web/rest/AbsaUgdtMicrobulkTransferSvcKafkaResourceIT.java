...
import ug.co.absa.microsrvc.transfer.bulk.IntegrationTest;
...
class AbsaUgdtMicrobulkTransferSvcKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/absa-ugdt-microbulk-transfer-svc-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/absa-ugdt-microbulk-transfer-svc-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/absa-ugdt-microbulk-transfer-svc-kafka/unregister"));
                return;
            }
        }
